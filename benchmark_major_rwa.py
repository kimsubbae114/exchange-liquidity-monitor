...
기존 benchmark_tool.py 의 틀을 따르되, 거래소를 넓히고
...
# ★종목과 종목 사이 쉬는 시간.
#   2.0 이면 한 라운드가 57초, 0.5 면 20초다(실측). 0.5 아래로는 더 안 빨라진다 —
#   거기서부터는 네트워크가 병목이다. 전 구간 api_fail 0 이었지만 단발 측정이라,
#   1분 간격으로 돌리며 차단이 나는지 지켜본다.
PACE_SEC = 0.5
