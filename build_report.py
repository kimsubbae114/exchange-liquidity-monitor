...
body{background:#0b1220;color:#e2e8f0;font-family:'Segoe UI',Arial,sans-serif;font-size:13px;
     padding:18px 20px 96px}   /* 아래 고정 바에 가리지 않게 */

/* ★화면 아래에 붙어 따라다니는 조작 바.
   표가 길어 스크롤하다 보면 위쪽 버튼이 사라진다 — 무엇을 보고 있는지
   놓치기 쉽다. 위가 아니라 아래에 두는 이유는 표 헤더가 이미
   상단에 sticky 로 붙어 있어 서로 겹치기 때문이다. */
.stickybar{position:fixed;left:0;right:0;bottom:0;z-index:40;
  background:rgba(11,18,32,.97);border-top:1px solid #24344d;
  box-shadow:0 -6px 18px rgba(0,0,0,.45);backdrop-filter:blur(6px)}
.sbin{max-width:1560px;margin:0 auto;padding:9px 20px;
  display:flex;gap:16px;flex-wrap:wrap;align-items:center}
.sbin b{font-size:11px;color:#64748b;font-weight:700;margin-right:5px}
.sbin .badge{font-size:10.5px;color:#64748b;flex:1 1 260px;min-width:0}
@media(max-width:860px){.sbin{gap:10px;padding:7px 12px}
  .sbin .badge{display:none}
...
<div class="note" id="feeTable" style="margin:8px 0 0;display:none"></div>
...
<div class="stickybar">
  <div class="sbin">
    <span><b>SIZE</b><span class="seg" id="segZ"></span></span>
...
function drawSpikeChart(){
  ...
  /* 위쪽 COST 버튼을 그대로 따른다 — 한 화면이 두 가지 값을 말하면 안 된다.
     (스파이크 표의 '초과분'은 기준선과 값에 같은 수수료가 더해져 상쇄되므로
      수수료와 무관하다. 그래서 표는 건드리지 않는다.) */
  const FEES = D.meta.fees || {};
  const adj = (v, y) => y == null ? null : y + (WITHFEE ? (FEES[v] || 0) : 0);

  let hi = 0, lo = Infinity;
  for (const v of vens) for (const y0 of blk.v[v]) {
    const y = adj(v, y0);
  ...
  const flat = [];
  for (const v of vens) for (const y0 of blk.v[v]) {
    const y = adj(v, y0); if (y != null && y > 0) flat.push(y);
  }
  flat.sort((a, b) => a - b);
